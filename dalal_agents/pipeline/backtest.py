...
async def run_backtest(
    ...
    _console.rule(
        Text(
            f"Backtest  ·  {ticker} ({exchange.value})  {start_date} → {end_date}",
            style="bold yellow",
        ),
        style="yellow",
    )
    ...
    sharpe = (
        float((returns.mean() / (returns.std() + 1e-9)) * (252**0.5)) if len(returns) >= 2 else 0.0
    )
    ...
    _console.print(
        f"  [bold]Cumulative return:[/bold] [{ret_color}]{cum_return:.2f}%[/{ret_color}]"
    )
